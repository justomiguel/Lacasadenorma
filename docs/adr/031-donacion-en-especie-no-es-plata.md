...
y su único uso es interno: ordenar el catálogo por lo que más pesa y que la familia decida qué pedir
primero. No se publica (decisión D3 de la especificación).
...
- Un ítem con valor estimado cargado y no publicado es un dato que existe en la base y no se ve en
  ninguna pantalla pública. Es deliberado y queda anotado en la frontera de `data-model.md`, para que
  no se lo descubra como un campo olvidado.
