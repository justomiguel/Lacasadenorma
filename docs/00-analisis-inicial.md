...
| **`loading-states`** (`sanky369/vibe-building-skills`) | instalada el 2026-09-15 | La Skill pública que más se acerca a revisar espera, vacío y error: inventaria cada superficie async, elige el indicador por latencia y exige `aria-busy` y región viva. MIT. En el punto donde pide skeleton o spinner se ignora (ADR-032, `BusyCue`); en el inventario y en el anuncio se sigue | `.agents/skills/loading-states/`, `skills-lock.json` (commiteados) |
| **`animate`** (`emilkowalski/skills`) | instalada el 2026-09-15 | La Skill pública de motion con mejor criterio de contención: decide si anima, con qué herramienta (CSS antes que una librería) y con valores exactos. MIT. Receta de scroll reveal con IntersectionObserver una sola vez. Donde pide clip-path, `--ease-out` paralelo o movimiento reducido «más suave», gana ADR-032 | `.agents/skills/animate/`, `skills-lock.json` (commiteados) |
...
| **`ui-ux-pro-max`** (`nextlevelbuilder/ui-ux-pro-max-skill`) | Tiene 119 reglas de UX (carga, vacío, foco al inválido), pero el flujo por defecto genera un design system y recomienda stacks `shadcn` / `html-tailwind`. El inventario de espera se cubre mejor con `loading-states`, sin ese empujón |
| **`ux-feedback`** (`sickn33/agentic-awesome-skills`) | Redistribuye StyleSeed: skeletons `animate-pulse`, toasts y "el cero se muestra como 0". Choca con ADR-032 y con la prohibición de mostrar un cero cuando el dato no está |
| **`gsap-scrolltrigger`** (`greensock/gsap-skills`) | El revelado editorial no justifica instalar GSAP: `animate` pide la herramienta más barata que funcione, y acá es CSS más IntersectionObserver |
| **`motion-framer`** y **`remotion-*`** | Empujan `motion.dev` o video. Una librería de springs para un fade viola la misma regla de herramienta barata |
| **`find-animation-opportunities`** (`emilkowalski/skills`) | Es de la misma familia y es de sólo lectura: propone recetas y no implementa. Se eligió `animate`, que escribe el movimiento |
...
| D5 | **Fotografías reales**, con crédito y autorización de la familia | En el relato se reserva el espacio; no se generan ni se usa stock. El catálogo puede mostrar una foto de referencia del tipo, etiquetada (ADR-043) |
