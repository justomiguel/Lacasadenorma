...
es el `alt`, el epígrafe y el título del tramo. Las vistas previas de la prensa viven en
`public/medios/`. `npm run check:fotos` recorre los JSON de los dos directorios y comprueba las dos
carpetas.
...
| `catalogo-fotos.json` | Ficha `/catalogo/[id]` | Fotos de referencia por título (ADR-043). El archivo vive en `public/fotos/catalogo/`. El epígrafe dice que es solamente ilustrativa y que no representa el objeto real. La foto subida desde el backoffice pisa esta. El ítem del fixture también tiene entrada |
...
El catálogo es otra cosa (ADR-043). Cada ficha puede mostrar una foto de referencia del tipo de
material, con epígrafe que dice que es solamente ilustrativa y que no representa el objeto real.
Si el equipo sube la foto real, esa pisa la de referencia. Si no hay ninguna, se reserva el hueco.
...
| Esa noche | **Los bomberos frente a la casa**, recorte apaisado a 2880×1620 — `incendio-bomberos-frente-hero.jpg` | Apertura de la home | El mismo fotograma que `incendio-bomberos-frente.jpg`, recortado a 16:9 y agrandado. El original mide 1220 px y en el héroe a sangrado se veía pixelado |
...
|---|---|---|
| `norma.fullName` | Publicado como **«Norma Edith Bedoya»**, que es lo que dice el material de la familia y la nota biográfica de junio de 2026. La crónica policial de esos días la nombra **«Bedolla»**, y un medio ubicó el incendio en Laguna Blanca. El apellido de una persona lo dice su familia, no un parte de prensa. Las notas **sí se enlazan** desde `/que-paso`; los títulos los escribimos nosotros, con Bedoya y Riacho He Hé, y no se copia el de la crónica | La familia |
