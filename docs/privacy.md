...
|---|---|---|---|
| Correo | `auth.users`, que gestiona Supabase Auth | La persona al registrarse, o el proveedor social si entra con una red (ADR-038) | **No.** Ninguna vista pública lo alcanza, y hay un privilegio de columna que lo hace imposible, no una consulta que se acuerda de omitirlo |
...
5. **El nombre y la foto de Google (o de cualquier otra red) no se copian al perfil** (ADR-038).
