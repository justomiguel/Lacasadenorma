...
export function Stat({
  ...
  const value = typeof amount === "string" ? amount : formatMoney(amount);
...
export function ProgressBar({
  ...
  const raisedText = formatMoney(raised);
    ...
    return (
      <div className={className}>
        ...
        <p className="mt-xs max-w-measure font-ui text-small text-ink-muted">
          recaudado. El objetivo todavía no está publicado, así que no mostramos un
          porcentaje.
        </p>
      </div>
    );
  ...
  const label = `${raisedText} recaudados de un objetivo de ${formatMoney(goal)}`;
  ...
  return (
    <div className={className}>
      ...
      <p className="mt-xs font-ui text-small text-ink-muted" data-figure>
        {formatPercentage(percent)} del objetivo
      </p>
    </div>
  );
}
...
export function TableAmount({ amount }: { amount: Money }) {
  return (
