 ...
 *
 * Cada test recorre `PAGINAS_PUBLICAS` en lugar de haber uno por página: son esos gotos
 * y una medición, y partirlos en un test por ruta agrega más arranque que cobertura. El
...
test.describe("revisión visual · ux.md §12", () => {
  test("las páginas públicas cumplen los criterios medibles en el viewport del proyecto", async ({
    page,
    request,
  }) => {
  ...
  test("las páginas públicas cumplen los criterios medibles en 360 px", async ({
    page,
    request,
  }) => {
    test.setTimeout(90_000);
