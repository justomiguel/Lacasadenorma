...
| III · Simplicidad | Cero dependencias nuevas. Sin cola de correo, sin captcha, sin motor de plantillas. OAuth usa el que Auth ya trae (ADR-039), no un SDK. El único patrón agregado es el puerto `EmailSender`, y existe porque hay dos implementaciones reales el primer día (Resend y la que registra y no manda) | ✅ |
