...
| Ruta | Tipo | Qué hace |
|---|---|---|
| `/cuenta/crear` | Página + formulario de cliente | Registro con correo y contraseña, y con las redes habilitadas (ADR-038) |
...
`/cuenta/oauth` es el mismo patrón para el salto a una red (ADR-038). Canjea el `code` de PKCE con
...
botón no existe (ADR-038). `check:secrets` sigue verificando que ninguna clave secreta lleve el
