...
| T2 | Se alteran cifras para ocultar un gasto | Los registros financieros no se borran: se anulan con motivo, y la anulación se audita. `audit_log` no tiene policies de `UPDATE` ni `DELETE` para ningún rol, y el `INSERT` no está disponible como privilegio de tabla: entra sólo por `public.record_audit()` | pgTAP: `delete` sobre `audit_log` falla incluso para `owner`; `insert` directo falla por falta de privilegio para los cuatro roles |
...
| T4 | XSS por contenido de administración | React escapa por defecto; el cuerpo de las novedades se renderiza desde Markdown restringido, **sin HTML crudo**; el editor visual (ADR-034) serializa a ese árbol y no hay `dangerouslySetInnerHTML`; un video es `<video>` alojado acá, no un `iframe` | Test unitario del renderizador con entrada `<script>`; parser de `![x](https://…)` sin nodo de media; regla de ESLint |
...
| T6 | SVG malicioso subido como foto o como video | Se rechaza `image/svg+xml` en la carga; fotos: JPEG, PNG, WebP y AVIF; videos: MP4 y WebM; el tipo se valida por contenido, no por extensión ni por el `Content-Type` que declara el cliente | Test de integración con un SVG renombrado a `.png` y a `.mp4` |
...
| Fidelidad parcial del shim local de Postgres | Falta GoTrue y Realtime. Una migración puede pasar local y fallar en el proyecto real, o al revés | `db push --dry-run` antes del primer push, y `db advisors --linked` como compuerta verdadera |
| La mutación y su entrada de auditoría no son atómicas | Son dos viajes a la base: si el segundo falla, el cambio queda sin rastro y la pantalla informa el error. Envolverlos en una transacción única exigiría una función SQL por operación, y esa duplicación del dominio en PL/pgSQL es peor que la ventana ([ADR-019](../../docs/adr/019-auditoria-por-funcion.md)) | Si aparece una operación donde el rastro sea un requisito legal y no operativo |
